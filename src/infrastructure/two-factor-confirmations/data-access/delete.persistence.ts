...
export async function deleteTwoFactorConfirmationByUser(userId: string): Promise<void> {
  await db.delete(twoFactorConfirmations).where(eq(twoFactorConfirmations.userId, userId));
}
