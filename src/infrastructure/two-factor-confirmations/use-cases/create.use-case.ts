...
export async function createTwoFactorConfirmationUseCase(
  ...
  try {
    const existingConfirmation = await context.getTwoFactorConfirmationByUser(data.userId);

    if (existingConfirmation) await context.deleteTwoFactorConfirmation(existingConfirmation.id);

    const confirmation = new TwoFactorConfirmationEntity({
      userId: data.userId,
    });

    await context.createTwoFactorConfirmation(TwoFactorConfirmationToCreateDto(confirmation));
  } catch (error) {
